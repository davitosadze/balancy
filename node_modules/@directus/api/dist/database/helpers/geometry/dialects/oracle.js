...
//#region src/database/helpers/geometry/dialects/oracle.ts
var GeometryHelperOracle = class extends GeometryHelper {
	isTrue(expression) {
	...
	asText(table, column, alias) {
		if (alias) return this.knex.raw("sdo_util.to_wktgeometry(??.??) as ??", [
			table,
			column,
			alias
		]);
	...
	asGeoJSON(table, column) {
		return this.knex.raw("sdo_util.to_geojson(??.??) as ??", [
			table,
			column,
			column
		]);
