...
async function runAst(originalAST, schema, accountability, options) {
	...
	async function run(collection, children, query, cases, accountability$1) {
		...
		let permissions = [];
		if (accountability$1 && !accountability$1.admin) {
			const policies = await fetchPolicies(accountability$1, {
				schema,
				knex
			});
		...
		for (const nestedNode of nestedNodes) {
			...
			if (nestedNode.type === "o2m") {
				...
				while (hasMore) {
					const node = merge({}, nestedNode, { query: {
						limit: env["RELATIONAL_BATCH_SIZE"],
						offset: batchCount * env["RELATIONAL_BATCH_SIZE"],
						page: null
					} });
					nestedItems = await runAst(node, schema, accountability$1, {
						knex,
						nested: true
					});
					if (nestedItems) {
				...
				const node = merge({}, nestedNode, { query: { limit: -1 } });
				nestedItems = await runAst(node, schema, accountability$1, {
					knex,
					nested: true
				});
