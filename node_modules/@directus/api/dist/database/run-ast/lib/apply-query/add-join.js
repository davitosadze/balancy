...
//#region src/database/run-ast/lib/apply-query/add-join.ts
function addJoin({ path, collection, aliasMap, rootQuery, schema, knex }) {
	...
	function followRelation(pathParts, parentCollection = collection, parentFields) {
		...
		*/
		...
		const existingAlias = parentFields ? aliasMap[`${parentFields}.${pathParts[0]}`]?.alias : aliasMap[pathParts[0]]?.alias;
		if (!existingAlias) {
			...
			if (relationType === "m2o") {
				...
				if (!pathScope) {
					throw new InvalidQueryError({ reason: `You have to provide a collection scope when sorting or filtering on a many-to-any item` });
				}
				rootQuery.leftJoin({ [alias]: pathScope }, (joinClause) => {
					joinClause.onVal(`${aliasedParentCollection}.${relation.meta.one_collection_field}`, "=", pathScope).andOn(`${aliasedParentCollection}.${relation.field}`, "=", knex.raw(getHelpers(knex).schema.castA2oPrimaryKey(), `${alias}.${schema.collections[pathScope].primary}`));
				});
				aliasMap[aliasKey].collection = pathScope;
				isJoinAdded = true;
			} else if (relationType === "o2a") {
				rootQuery.leftJoin({ [alias]: relation.collection }, (joinClause) => {
					joinClause.onVal(`${alias}.${relation.meta.one_collection_field}`, "=", parentCollection).andOn(`${alias}.${relation.field}`, "=", knex.raw(getHelpers(knex).schema.castA2oPrimaryKey(), `${aliasedParentCollection}.${schema.collections[parentCollection].primary}`));
				});
