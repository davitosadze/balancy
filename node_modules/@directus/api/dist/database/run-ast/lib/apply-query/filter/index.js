			...
			*/
			...
			if (filterPath.length > 1 || !(key.includes("(") && key.includes(")")) && schema.collections[collection$1]?.fields[key]?.type === "alias") {
				...
				if (relationType === "o2m" || relationType === "o2a") {
					...
					if (relationType === "o2a") {
						pkField = knex$1.raw(getHelpers(knex$1).schema.castA2oPrimaryKey(), [pkField]);
					}
				...
				if (filterPath.includes("_none") || filterPath.includes("_some")) {
					throw new InvalidQueryError({ reason: `"${filterPath.includes("_none") ? "_none" : "_some"}" can only be used with top level relational alias field` });
				}
				...
				validateOperator(type, filterOperator, special);
				const aliasedCollection = aliasMap[""]?.alias || collection$1;
				applyOperator(knex$1, dbQuery, schema, `${aliasedCollection}.${filterPath[0]}`, filterOperator, filterValue, logical, collection$1);
			}
		}
	}
}
