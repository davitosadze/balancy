...
function mergeWithParentItems(schema, nestedItem, parentItem, nestedNode, fieldAllowed) {
	...
	if (nestedNode.type === "m2o") {
		...
		for (const nestedItem$1 of nestedItems) {
			...
			for (const parentItem$1 of parentsByForeignKey.get(nestedPK)) {
				parentItem$1[nestedNode.fieldKey] = nestedItem$1;
			}
		}
		...
		for (const parentItem$1 of parentItems) {
			if (!parentItem$1[parentRelationField]) {
				parentItem$1[parentRelationField] = [];
			}
		...
		for (const [index, parentItem$1] of parentItems.entries()) {
			...
			if (nestedNode.query.page && nestedNode.query.page > 1) {
				parentItem$1[nestedNode.fieldKey] = parentItem$1[nestedNode.fieldKey].slice(limit * (nestedNode.query.page - 1));
			}
			if (nestedNode.query.offset && nestedNode.query.offset >= 0) {
				parentItem$1[nestedNode.fieldKey] = parentItem$1[nestedNode.fieldKey].slice(nestedNode.query.offset);
			}
			if (limit !== -1) {
				parentItem$1[nestedNode.fieldKey] = parentItem$1[nestedNode.fieldKey].slice(0, limit);
			}
