...
//#region src/database/run-ast/utils/with-preprocess-bindings.ts
function withPreprocessBindings(knex, dbQuery) {
	const schemaHelper = getHelpers(knex).schema;
	dbQuery.client = new Proxy(dbQuery.client, { get(target, prop, receiver) {
		if (prop === "query") {
