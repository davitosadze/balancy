...
var MSSQLErrorCodes = /* @__PURE__ */ function(MSSQLErrorCodes$1) {
	...
	MSSQLErrorCodes$1[MSSQLErrorCodes$1["UNIQUE_VIOLATION_INDEX"] = 2601] = "UNIQUE_VIOLATION_INDEX";
	MSSQLErrorCodes$1[MSSQLErrorCodes$1["UNIQUE_VIOLATION_CONSTRAINT"] = 2627] = "UNIQUE_VIOLATION_CONSTRAINT";
...
async function extractError(error, data) {
	switch (error.number) {
		...
		case MSSQLErrorCodes.UNIQUE_VIOLATION_INDEX: return await uniqueViolation(error);
	...
	return error;
	async function uniqueViolation(error$1) {
		/**
		* NOTE:
		...
		*/
		...
		const quoteMatches = error$1.message.match(betweenQuotes);
		const parenMatches = error$1.message.match(betweenParens);
		if (!quoteMatches || !parenMatches) return error$1;
		const [keyNameMatchIndex, collectionNameMatchIndex] = error$1.number === MSSQLErrorCodes.UNIQUE_VIOLATION_INDEX ? [1, 0] : [0, 1];
