...
//#region src/database/migrations/20230526A-migrate-translation-strings.ts
...
function transformStringsOldFormat(newStrings) {
	...
	return Object.entries(keyCache).map(([key, translations]) => ({
		key,
		translations
	}));
}
...
async function down(knex) {
	...
	if (settingsId?.id && data) {
		...
		await knex("directus_settings").where({ id: settingsId.id }).update({ translation_strings: JSON.stringify(oldTranslationStrings) });
	}
