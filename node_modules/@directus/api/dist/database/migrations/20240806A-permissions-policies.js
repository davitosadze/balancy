...
async function up(knex) {
	...
	if (roles.length > 0) {
		await processChunk(roles, 100, async (chunk$1) => {
	...
	try {
		...
		const foreignConstraint = foreignKeys.find((foreign) => foreign.foreign_key_table === "directus_roles" && foreign.column === "role")?.constraint_name || undefined;
		await knex.schema.alterTable("directus_permissions", (table) => {
	...
	await processChunk(policyAttachments, 100, async (chunk$1) => {
