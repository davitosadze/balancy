...
//#region src/database/migrations/20211007A-update-presets.ts
async function up(knex) {
	await knex.schema.alterTable("directus_presets", (table) => {
		table.json("filter");
	});
	const presets = await knex.select("id", "filters", "layout_query").from("directus_presets");
	for (const preset of presets) {
		if (preset.filters) {
			...
			const newFilter = { _and: [] };
			for (const oldFilter of oldFilters) {
				if (oldFilter.key === "hide-archived") continue;
				newFilter._and.push({ [oldFilter.field]: { ["_" + oldFilter.operator]: oldFilter.value } });
			}
			if (newFilter._and.length > 0) {
				await knex("directus_presets").update({ filter: JSON.stringify(newFilter) }).where("id", "=", preset.id);
			}
		}
		if (preset.layout_query) {
			...
			await knex("directus_presets").update({ layout_query: JSON.stringify(layoutQuery) }).where("id", "=", preset.id);
		}
	}
	await knex.schema.alterTable("directus_presets", (table) => {
...
async function down(knex) {
	...
	const presets = await knex.select("id", "filter", "layout_query").from("directus_presets");
	for (const preset of presets) {
		if (preset.filter) {
			...
			if (oldFilters.length > 0) {
				await knex("directus_presets").update({ filters: JSON.stringify(oldFilters) }).where("id", "=", preset.id);
			}
		}
		if (preset.layout_query) {
			const layoutQuery = typeof preset.layout_query === "string" ? parseJSON(preset.layout_query) : preset.layout_query;
