...
//#region src/database/migrations/20210927A-replace-fields-group.ts
async function up(knex) {
	...
	for (const { id, group } of groupsInUse) {
		await knex("directus_fields").update({ group: groupMap.get(group) }).where({ id });
	}
}
async function down(knex) {
	const fieldsThatUseAGroup = await knex.select("id", "collection", "group").from("directus_fields").whereNotNull("group");
	if (fieldsThatUseAGroup.length === 0) return;
	const groupMap = new Map();
	for (const { collection, group } of fieldsThatUseAGroup) {
		const { id } = await knex.select("id").from("directus_fields").where({
			collection,
			field: group
		}).first();
