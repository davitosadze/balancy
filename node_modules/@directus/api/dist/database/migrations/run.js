...
async function run(database, direction, log = true) {
	...
	const migrations = [...migrationFiles.map((path$1) => parseFilePath(path$1)), ...customMigrationFiles.map((path$1) => parseFilePath(path$1, true))].sort((a, b) => a.version > b.version ? 1 : -1);
	...
	async function up() {
		...
		const { up: up$1 } = await import(`file://${nextVersion.file}`);
		if (!up$1) {
	...
	async function down() {
		...
		const { down: down$1 } = await import(`file://${migration.file}`);
		if (!down$1) {
	...
	async function latest() {
		...
		for (const migration of migrations) {
			if (migration.completed === false) {
				...
				const { up: up$1 } = getModuleDefault(await import(`file://${migration.file}`));
				if (!up$1) {
