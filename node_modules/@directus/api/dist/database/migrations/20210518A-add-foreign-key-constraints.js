...
//#region src/database/migrations/20210518A-add-foreign-key-constraints.ts
async function up(knex) {
	const logger = useLogger();
	const inspector = createInspector(knex);
	const foreignKeys = await inspector.foreignKeys();
	const relations = await knex.select("id", "many_collection", "many_field", "one_collection").from("directus_relations");
	...
	for (const constraint of constraintsToAdd) {
		...
		if (await inspector.hasTable(constraint.many_collection) === false || await inspector.hasTable(constraint.one_collection) === false) {
		...
		const rowsWithIllegalFKValues = await knex.select(`main.${currentPrimaryKeyField}`).from({ main: constraint.many_collection }).leftJoin({ related: constraint.one_collection }, `main.${constraint.many_field}`, `related.${relatedPrimaryKeyField}`).whereNull(`related.${relatedPrimaryKeyField}`);
		if (rowsWithIllegalFKValues.length > 0) {
			const ids = rowsWithIllegalFKValues.map((row) => row[currentPrimaryKeyField]);
			try {
				await knex(constraint.many_collection).update({ [constraint.many_field]: null }).whereIn(currentPrimaryKeyField, ids);
			} catch {
		...
		try {
			await knex.schema.alterTable(constraint.many_collection, (table) => {
				if (columnInfo.data_type !== relatedColumnInfo.data_type && columnInfo.data_type === "int" && relatedColumnInfo.data_type === "int unsigned") {
				...
				const builder = table.foreign(constraint.many_field, indexName).references(relatedPrimaryKeyField).inTable(constraint.one_collection);
...
async function down(knex) {
	const logger = useLogger();
	const relations = await knex.select("many_collection", "many_field", "one_collection").from("directus_relations");
