	...
	for (let index = 0; index < fields.length; index++) {
		...
		if (fieldKey.includes(".") && fieldKey.split(".")[0] === "*") {
			...
			if (allowedFields.includes("*")) {
				...
				relationalFields = allowedFields.filter((fieldKey$1) => getRelation(context.schema.relations, options.collection, fieldKey$1) !== undefined);
			}
			...
			const aliasFields = Object.keys(options.alias ?? {}).map((fieldKey$1) => {
				...
				if (relationalFields.includes(name)) {
					return `${fieldKey$1}.${parts.slice(1).join(".")}`;
				}
				return fieldKey$1;
			});
