...
//#region src/metrics/lib/create-metrics.ts
		...
		if (isPM2 && aggregates.size !== 0) {
			...
			for (const app of apps) {
				if (aggregates.has(app.pid)) {
					aggregate$1.push(aggregates.get(app.pid));
				}
			}
			if (aggregate$1.length !== 0) {
				return AggregatorRegistry.aggregate(aggregate$1).metrics();
			}
		}
