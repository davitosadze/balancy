...
//#region src/operations/trigger/index.ts
var trigger_default = defineOperationApi({
	id: "trigger",
	handler: async ({ flow, payload, iterationMode, batchSize }, context) => {
		...
		if (Array.isArray(payloadObject)) {
			if (iterationMode === "serial") {
				const result = [];
				for (const payload$1 of payloadObject) {
					result.push(await flowManager.runOperationFlow(flow, payload$1, omit(context, "data")));
				}
				return result;
			}
			if (iterationMode === "batch") {
				...
				for (let i = 0; i < payloadObject.length; i += size) {
					...
					const batchResults = await Promise.all(batch.map((payload$1) => {
						return flowManager.runOperationFlow(flow, payload$1, omit(context, "data"));
					}));
			...
			if (iterationMode === "parallel" || !iterationMode) {
				return await Promise.all(payloadObject.map((payload$1) => {
					return flowManager.runOperationFlow(flow, payload$1, omit(context, "data"));
				}));
			}
		}
