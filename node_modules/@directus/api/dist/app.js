...
async function createApp() {
	...
	if (env["CROSS_ORIGIN_OPENER_POLICY_ENABLED"]) {
		app.use(helmet.crossOriginOpenerPolicy({ policy: env["CROSS_ORIGIN_OPENER_POLICY"] ?? "same-origin-allow-popups" }));
	}
	if (env["HSTS_ENABLED"]) {
	...
	if (env["SERVE_APP"]) {
		...
		const htmlWithVars = html.replace(/<base \/>/, `<base href="${adminUrl.toString({ rootRelative: true })}/" />`).replace("<!-- directus-embed-head -->", embeds.head).replace("<!-- directus-embed-body -->", embeds.body);
	...
	if (env["RATE_LIMITER_GLOBAL_ENABLED"] === true) {
		app.use(rate_limiter_global_default);
	}
	if (env["RATE_LIMITER_ENABLED"] === true) {
		app.use(rate_limiter_ip_default);
	}
