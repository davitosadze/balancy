	...
	*/
	async getRoom(uid) {
		...
		if (!room) {
			...
			room = await store(async (store$1) => {
				if (!await store$1.has("uid")) return;
				const collection = await store$1.get("collection");
				const item = await store$1.get("item");
				const version = await store$1.get("version");
				const changes = await store$1.get("changes");
				const room$1 = new Room(uid, collection, item, version, changes, this.messenger);
		...
		this.onUpdateHandler = async (meta) => {
			...
			try {
				...
				const result = await (async () => {
					if (this.version) {
						const service$1 = getService("directus_versions", { schema });
						const versionData = await service$1.readOne(this.version);
				...
				for (const client of clients) {
					this.send(client.uid, { action: ACTION.SERVER.SAVE });
				}
			} catch (err) {
