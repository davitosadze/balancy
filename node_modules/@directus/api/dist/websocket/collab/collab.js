...
var CollabHandler = class {
	...
	*/
	async onJoin(client, message) {
		if (client.accountability?.share) {
			throw new ForbiddenError({ reason: "Collaborative editing is not supported for shares" });
		}
		const schema = await getSchema();
	...
	async onLeave(client, message) {
		if (message?.room) {
			...
			if (!room || !await room.hasClient(client.uid)) {
				throw new ForbiddenError({ reason: `No access to room "${message.room}" or it does not exist` });
			}
	...
	*/
	async onUpdate(client, message) {
		...
		const room = await this.roomManager.getRoom(message.room);
		if (!room || !await room.hasClient(client.uid)) {
			throw new ForbiddenError({ reason: `No access to room ${message.room} or room does not exist` });
		}
		...
		if (message.changes !== undefined) {
			...
			if (!focus || focus !== message.field) {
				throw new ForbiddenError({ reason: `Cannot update field ${message.field} without focusing on it first` });
			}
			...
			if (currentFocuser && currentFocuser !== client.uid) {
				throw new ForbiddenError({ reason: `Field ${message.field} is already focused by another user` });
			}
	...
	*/
	async onDiscard(client, message) {
		...
		if (!allowedFields || allowedFields.length === 0) {
			throw new ForbiddenError({ reason: `No permission to discard fields or item does not exist` });
		}
	...
	async checkFieldsAccess(client, room, fields, errorAction, options = {}) {
		...
		for (const field of fieldsArray) {
			...
			if (!fieldExists || allowedFields && !isFieldAllowed(allowedFields, field)) {
				throw new ForbiddenError({ reason: `No permission to ${errorAction} field ${field} or field does not exist` });
			}
		}
	}
