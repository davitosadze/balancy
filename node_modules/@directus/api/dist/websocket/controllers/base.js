...
const TOKEN_CHECK_INTERVAL = 15 * 60 * 1e3;
...
var SocketController = class {
	...
	getRateLimiter() {
		...
		if (toBoolean(env["RATE_LIMITER_ENABLED"]) === true) {
			return createRateLimiter("RATE_LIMITER", { keyPrefix: "websocket" });
		}
		return null;
	}
	...
	async handleAuthRequest(client, message) {
		try {
			...
			*/
			...
			const _error = error instanceof WebSocketError ? error : new WebSocketError("auth", "AUTH_FAILED", "Authentication failed.", message.uid);
