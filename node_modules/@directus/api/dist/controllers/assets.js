...
router.get("/:pk/:filename?", async_handler_default(async (req, res, next) => {
	...
	const savedAssetSettings = await database.select("storage_asset_presets", "storage_asset_transform").from("directus_settings").first();
	...
	if ("transforms" in transformation) {
		...
		if (transforms.length > Number(env["ASSETS_TRANSFORM_MAX_OPERATIONS"])) {
			throw new InvalidQueryError({ reason: `"transforms" Parameter is only allowed ${env["ASSETS_TRANSFORM_MAX_OPERATIONS"]} transformations` });
		}
		transforms.forEach((transform$1) => {
			const name = transform$1[0];
			if (!TransformationMethods.includes(name)) {
				throw new InvalidQueryError({ reason: `"transforms" Parameter does not allow "${name}" as a transformation` });
			}
		});
	...
	if (Object.keys(transformation).length === 0 || "transforms" in transformation && transformation["transforms"].length === 0) {
		return next();
	}
	if (assetSettings.storage_asset_transform === "all") {
		...
		if (transformation["key"] && systemKeys.includes(transformation["key"]) && Object.keys(transformation).length === 1) {
			return next();
		}
