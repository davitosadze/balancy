...
var ServerService = class {
	...
	async serverInfo() {
		...
		const projectInfo = await this.settingsService.readSingleton({ fields: [
			"project_name",
			"project_descriptor",
			"project_logo",
			"project_color",
		...
		if (this.accountability?.user) {
			...
			if (FILE_UPLOADS.MAX_CONCURRENCY && FILE_UPLOADS.MAX_CONCURRENCY !== Infinity) {
				info["uploads"] = { maxConcurrency: FILE_UPLOADS.MAX_CONCURRENCY };
			}
	...
	async health() {
		...
		async function testDatabase() {
			...
			if (Number(checks[`${client}:responseTime`][0].observedValue) > checks[`${client}:responseTime`][0].threshold && checks[`${client}:responseTime`][0].status !== "error") {
		...
		async function testCache() {
			...
			try {
				...
				if (checks["cache:responseTime"][0].observedValue > checks["cache:responseTime"][0].threshold && checks["cache:responseTime"][0].status !== "error") {
		...
		async function testRateLimiter() {
			...
			try {
				...
				if (checks["rateLimiter:responseTime"][0].observedValue > checks["rateLimiter:responseTime"][0].threshold && checks["rateLimiter:responseTime"][0].status !== "error") {
		...
		async function testRateLimiterGlobal() {
			...
			try {
				...
				if (checks["rateLimiterGlobal:responseTime"][0].observedValue > checks["rateLimiterGlobal:responseTime"][0].threshold && checks["rateLimiterGlobal:responseTime"][0].status !== "error") {
		...
		async function testStorage() {
			...
			for (const location of toArray(env["STORAGE_LOCATIONS"])) {
				...
				try {
					...
					if (Number(checks[`storage:${location}:responseTime`][0].observedValue) > checks[`storage:${location}:responseTime`][0].threshold && checks[`storage:${location}:responseTime`][0].status !== "error") {
