...
import { getRelationType as getRelationType$1 } from "./get-relation-type.js";
...
//#region src/services/graphql/resolvers/system.ts
const env = useEnv();
function injectSystemResolvers(gql, schemaComposer, { CreateCollectionTypes, ReadCollectionTypes, UpdateCollectionTypes }, schema) {
	...
	if (gql.accountability?.user) {
		ServerInfo.addFields({
			...
			rateLimitGlobal: env["RATE_LIMITER_GLOBAL_ENABLED"] ? { type: new GraphQLObjectType({
			...
			websocket: toBoolean(env["WEBSOCKETS_ENABLED"]) ? { type: new GraphQLObjectType({
				name: "server_info_websocket",
				fields: {
					rest: { type: toBoolean(env["WEBSOCKETS_REST_ENABLED"]) ? new GraphQLObjectType({
						name: "server_info_websocket_rest",
						fields: {
							authentication: { type: new GraphQLEnumType({
					...
					graphql: { type: toBoolean(env["WEBSOCKETS_GRAPHQL_ENABLED"]) ? new GraphQLObjectType({
					...
					heartbeat: { type: toBoolean(env["WEBSOCKETS_HEARTBEAT_ENABLED"]) ? GraphQLInt : GraphQLBoolean }
				}
			}) } : GraphQLBoolean,
			queryLimit: { type: new GraphQLObjectType({
	...
	if (env["OPENAPI_ENABLED"] !== false) {
		schemaComposer.Query.addFields({ server_specs_oas: {
			type: GraphQLJSON,
	...
	if (env["GRAPHQL_INTROSPECTION"] !== false) {
		schemaComposer.Query.addFields({ server_specs_graphql: {
			type: GraphQLString,
			args: { scope: new GraphQLEnumType({
	...
	if ("directus_fields" in schema.read.collections) {
		...
		schemaComposer.Query.addFields({
			...
			fields_in_collection: {
				type: new GraphQLNonNull(new GraphQLList(new GraphQLNonNull(Field.getType()))),
				args: { collection: new GraphQLNonNull(GraphQLString) },
				resolve: dedupeResolver(async (_, args) => {
					const service = new FieldsService({
						accountability: gql.accountability,
						schema: gql.schema
	...
	if ("directus_relations" in schema.read.collections) {
		const Relation = getRelationType$1(schemaComposer, schema, "read");
		schemaComposer.Query.addFields({
			relations: {
	...
	if ("directus_users" in schema.read.collections) {
		schemaComposer.Query.addFields({ users_me: {
	...
	if ("directus_permissions" in schema.read.collections) {
		schemaComposer.Query.addFields({ permissions_me: {
	...
	if ("directus_roles" in schema.read.collections) {
		schemaComposer.Query.addFields({ roles_me: {
	...
	if ("directus_policies" in schema.read.collections) {
		schemaComposer.Query.addFields({ policies_me_globals: {
	...
	if ("directus_users" in schema.update.collections && gql.accountability?.user) {
		schemaComposer.Mutation.addFields({ update_users_me: {
			type: ReadCollectionTypes["directus_users"],
			args: { data: toInputObjectType(UpdateCollectionTypes["directus_users"]) },
	...
	if ("directus_users" in schema.create.collections) {
		schemaComposer.Mutation.addFields({ users_invite: {
			type: GraphQLBoolean,
			args: {
