...
//#region src/services/graphql/resolvers/system-admin.ts
function resolveSystemAdmin(gql, schema, schemaComposer) {
	...
	schemaComposer.Mutation.addFields({
		create_collections_item: {
			type: Collection,
			args: {
				data: toInputObjectType(Collection, { postfix: "_input" }).addFields({ fields: [toInputObjectType(Field, { postfix: "_input" }).NonNull] }).NonNull,
			...
			resolve: async (_, args) => {
				...
				const collectionKey = await collectionsService.createOne(args["data"], { attemptConcurrentIndex: Boolean(args["concurrentIndexCreation"]) });
		...
		update_collections_item: {
			...
			args: {
				...
				data: toInputObjectType(Collection, { postfix: "_input" }).removeField(["collection", "schema"]).NonNull
			},
			resolve: async (_, args) => {
				const collectionsService = new CollectionsService({
					accountability: gql.accountability,
		...
		delete_collections_item: {
			type: schemaComposer.createObjectTC({
				...
				fields: { collection: GraphQLString }
			}),
	...
	schemaComposer.Mutation.addFields({
		create_fields_item: {
			...
			resolve: async (_, args) => {
				...
				await service.createField(args["collection"], args["data"], undefined, { attemptConcurrentIndex: Boolean(args["concurrentIndexCreation"]) });
		...
		update_fields_items: {
			...
			resolve: async (_, args) => {
				...
				await service.updateFields(args["collection"], args["data"], { attemptConcurrentIndex: Boolean(args["concurrentIndexCreation"]) });
				return await service.readOne(args["collection"], args["field"]);
			}
		},
		delete_fields_item: {
	...
	schemaComposer.Mutation.addFields({
		create_relations_item: {
			type: Relation,
			args: { data: toInputObjectType(Relation, { postfix: "_input" }).NonNull },
			resolve: async (_, args) => {
	...
	const Extension = schemaComposer.createObjectTC({
		...
		fields: {
			...
			meta: schemaComposer.createObjectTC({
				...
				fields: { enabled: GraphQLBoolean }
			})
		}
	});
	schemaComposer.Query.addFields({ extensions: {
	...
	schemaComposer.Mutation.addFields({ update_extensions_item: {
		type: Extension,
		args: {
			id: GraphQLID,
			data: toInputObjectType(schemaComposer.createObjectTC({
				name: "update_directus_extensions_input",
				fields: { meta: schemaComposer.createObjectTC({
