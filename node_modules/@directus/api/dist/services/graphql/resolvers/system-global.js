...
//#region src/services/graphql/resolvers/system-global.ts
const env = useEnv();
/**
* Globally available mutations
*/
function globalResolvers(gql, schemaComposer) {
	...
	schemaComposer.Mutation.addFields({
			...
			resolve: async (_, args, { req, res }) => {
				...
				if (mode === "json") {
					...
					if (isDirectusJWT(token)) {
						const payload$1 = verifyAccessJWT(token, getSecret());
						currentRefreshToken = payload$1.session;
					}
				}
				if (!currentRefreshToken) {
					throw new InvalidPayloadError({ reason: `The refresh token is required in either the payload or cookie` });
				}
				const { accessToken, refreshToken, expires } = await authenticationService.refresh(currentRefreshToken, { session: mode === "session" });
				const payload = { expires };
				if (mode === "json") {
					payload.refresh_token = refreshToken;
					payload.access_token = accessToken;
		...
		users_me_tfa_generate: {
			...
			args: { password: new GraphQLNonNull(GraphQLString) },
		...
		users_me_tfa_disable: {
			type: GraphQLBoolean,
			args: { otp: new GraphQLNonNull(GraphQLString) },
			resolve: async (_, args) => {
				if (!gql.accountability?.user) return null;
				const service = new TFAService({
					accountability: gql.accountability,
		...
		utils_hash_generate: {
			type: GraphQLString,
			args: { string: new GraphQLNonNull(GraphQLString) },
			resolve: async (_, args) => {
		...
		utils_sort: {
			...
			resolve: async (_, args) => {
				...
				const { item, to } = args;
				await service.sort(args["collection"], {
					item,
					to
				});
				return true;
		...
		utils_revert: {
			type: GraphQLBoolean,
			args: { revision: new GraphQLNonNull(GraphQLID) },
			resolve: async (_, args) => {
		...
		users_register_verify: {
			type: GraphQLBoolean,
			args: { token: new GraphQLNonNull(GraphQLString) },
			resolve: async (_, args) => {
				const service = new UsersService({
					accountability: null,
					schema: gql.schema
