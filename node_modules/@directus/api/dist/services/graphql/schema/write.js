...
//#region src/services/graphql/schema/write.ts
function getWritableTypes(gql, schemaComposer, schema, inconsistentFields, ReadCollectionTypes) {
	...
	for (const collection of Object.values(schema.create.collections)) {
		...
		if (Object.keys(creatableFields).length > 0) {
			const resolverDefinition = {
				...
				type: collectionIsReadable ? new GraphQLNonNull(new GraphQLList(new GraphQLNonNull(ReadCollectionTypes[collection.collection].getType()))) : GraphQLBoolean,
		...
		if (Object.keys(updatableFields).length > 0) {
			if (collection.singleton) {
				UpdateCollectionTypes[collection.collection].addResolver({
					...
					type: collectionIsReadable ? ReadCollectionTypes[collection.collection] : GraphQLBoolean,
					args: { data: toInputObjectType(UpdateCollectionTypes[collection.collection]).setTypeName(`update_${collection.collection}_input`).NonNull },
					resolve: async ({ args, info }) => await resolveMutation(gql, args, info)
				});
			} else {
				UpdateCollectionTypes[collection.collection].addResolver({
					...
					type: collectionIsReadable ? new GraphQLNonNull(new GraphQLList(new GraphQLNonNull(ReadCollectionTypes[collection.collection].getType()))) : GraphQLBoolean,
					args: {
						...collectionIsReadable ? ReadCollectionTypes[collection.collection].getResolver(collection.collection).getArgs() : {},
	...
	DeleteCollectionTypes["many"] = schemaComposer.createObjectTC({
		name: `delete_many`,
		fields: { ids: new GraphQLNonNull(new GraphQLList(GraphQLID)) }
	});
	DeleteCollectionTypes["one"] = schemaComposer.createObjectTC({
		name: `delete_one`,
		fields: { id: new GraphQLNonNull(GraphQLID) }
	});
	for (const collection of Object.values(schema.delete.collections)) {
		DeleteCollectionTypes["many"].addResolver({
			...
			args: { ids: new GraphQLNonNull(new GraphQLList(GraphQLID)) },
			resolve: async ({ args, info }) => await resolveMutation(gql, args, info)
		});
