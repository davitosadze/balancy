//#region src/services/graphql/schema/parse-args.ts
/**
...
function parseArgs(args, variableValues) {
	...
	const parse = (node) => {
		switch (node.kind) {
			...
			case "ObjectValue": return Object.fromEntries(node.fields.map((node$1) => [node$1.name.value, parse(node$1.value)]));
			case "NullValue": return null;
			case "StringValue": return String(node.value);
			case "IntValue":
			case "FloatValue": return Number(node.value);
			case "BooleanValue": return Boolean(node.value);
			case "EnumValue":
			default: return "value" in node ? node.value : null;
		}
	};
