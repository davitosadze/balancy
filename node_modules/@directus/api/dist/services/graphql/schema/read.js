...
//#region src/services/graphql/schema/read.ts
/**
* Create readable types and attach resolvers for each. Also prepares full filter argument structures
*/
async function getReadableTypes(gql, schemaComposer, schema, inconsistentFields) {
	...
	const IDFilterOperators = schemaComposer.createInputTC({
		...
		fields: {
			_eq: { type: GraphQLID },
			_neq: { type: GraphQLID },
			_contains: { type: GraphQLID },
			_icontains: { type: GraphQLID },
			_ncontains: { type: GraphQLID },
			_starts_with: { type: GraphQLID },
			_nstarts_with: { type: GraphQLID },
			_istarts_with: { type: GraphQLID },
			_nistarts_with: { type: GraphQLID },
			_ends_with: { type: GraphQLID },
			_nends_with: { type: GraphQLID },
			_iends_with: { type: GraphQLID },
			_niends_with: { type: GraphQLID },
			_in: { type: new GraphQLList(GraphQLID) },
			_nin: { type: new GraphQLList(GraphQLID) },
	...
	const StringFilterOperators = schemaComposer.createInputTC({
		...
		fields: {
			...
			_istarts_with: { type: GraphQLString },
			_nistarts_with: { type: GraphQLString },
			_ends_with: { type: GraphQLString },
			_nends_with: { type: GraphQLString },
			_iends_with: { type: GraphQLString },
			_niends_with: { type: GraphQLString },
			_in: { type: new GraphQLList(GraphQLString) },
			_nin: { type: new GraphQLList(GraphQLString) },
			_null: { type: GraphQLBoolean },
			_nnull: { type: GraphQLBoolean },
			_empty: { type: GraphQLBoolean },
			_nempty: { type: GraphQLBoolean }
	...
	const DateFilterOperators = schemaComposer.createInputTC({
		...
		fields: {
			...
			_in: { type: new GraphQLList(GraphQLString) },
			_nin: { type: new GraphQLList(GraphQLString) },
			_between: { type: new GraphQLList(GraphQLStringOrFloat) },
			_nbetween: { type: new GraphQLList(GraphQLStringOrFloat) }
		}
	});
	const NumberFilterOperators = schemaComposer.createInputTC({
		name: "number_filter_operators",
		fields: {
			_eq: { type: GraphQLStringOrFloat },
			_neq: { type: GraphQLStringOrFloat },
			_in: { type: new GraphQLList(GraphQLStringOrFloat) },
			_nin: { type: new GraphQLList(GraphQLStringOrFloat) },
			_gt: { type: GraphQLStringOrFloat },
			_gte: { type: GraphQLStringOrFloat },
			_lt: { type: GraphQLStringOrFloat },
			_lte: { type: GraphQLStringOrFloat },
			_null: { type: GraphQLBoolean },
			_nnull: { type: GraphQLBoolean },
			_between: { type: new GraphQLList(GraphQLStringOrFloat) },
			_nbetween: { type: new GraphQLList(GraphQLStringOrFloat) }
	...
	const BigIntFilterOperators = schemaComposer.createInputTC({
		...
		fields: {
			_eq: { type: GraphQLBigInt },
			_neq: { type: GraphQLBigInt },
			_in: { type: new GraphQLList(GraphQLBigInt) },
			_nin: { type: new GraphQLList(GraphQLBigInt) },
			_gt: { type: GraphQLBigInt },
			_gte: { type: GraphQLBigInt },
			_lt: { type: GraphQLBigInt },
			_lte: { type: GraphQLBigInt },
			_null: { type: GraphQLBoolean },
			_nnull: { type: GraphQLBoolean },
			_between: { type: new GraphQLList(GraphQLBigInt) },
			_nbetween: { type: new GraphQLList(GraphQLBigInt) }
		}
	});
	const GeometryFilterOperators = schemaComposer.createInputTC({
		name: "geometry_filter_operators",
		fields: {
			_eq: { type: GraphQLGeoJSON },
			_neq: { type: GraphQLGeoJSON },
			_intersects: { type: GraphQLGeoJSON },
			_nintersects: { type: GraphQLGeoJSON },
			_intersects_bbox: { type: GraphQLGeoJSON },
			_nintersects_bbox: { type: GraphQLGeoJSON },
			_null: { type: GraphQLBoolean },
			_nnull: { type: GraphQLBoolean }
		}
	});
	...
	const DateFunctionFilterOperators = schemaComposer.createInputTC({
		...
		fields: {
			year: { type: NumberFilterOperators },
			month: { type: NumberFilterOperators },
			week: { type: NumberFilterOperators },
			day: { type: NumberFilterOperators },
			weekday: { type: NumberFilterOperators }
		}
	});
	const TimeFunctionFilterOperators = schemaComposer.createInputTC({
		name: "time_function_filter_operators",
		fields: {
			hour: { type: NumberFilterOperators },
			minute: { type: NumberFilterOperators },
			second: { type: NumberFilterOperators }
		}
	});
	...
	for (const collection of Object.values(schema.read.collections)) {
			...
			fields: Object.values(collection.fields).reduce((acc, field) => {
				...
				switch (graphqlType) {
					...
					case GraphQLID:
						...
						break;
					default: filterOperatorType = StringFilterOperators;
				}
				...
				if (field.type === "time") {
					acc[`${field.field}_func`] = { type: TimeFunctionFilterOperators };
				}
				if (field.type === "dateTime" || field.type === "timestamp") {
					acc[`${field.field}_func`] = { type: DateTimeFunctionFilterOperators };
				}
				if (field.type === "json" || field.type === "alias") {
					acc[`${field.field}_func`] = { type: CountFunctionFilterOperators };
				}
				return acc;
			}, {})
		});
		...
		const resolver = {
			...
			type: collection.singleton ? ReadCollectionTypes[collection.collection] : new GraphQLNonNull(new GraphQLList(new GraphQLNonNull(ReadCollectionTypes[collection.collection].getType()))),
		...
		if (collection.singleton === false) {
			...
			resolver.args = { version: GraphQLString };
		}
		...
		if (gql.scope === "items") {
			VersionCollectionTypes[collection.collection].addResolver({
				...
				args: collection.singleton ? { version: new GraphQLNonNull(GraphQLString) } : {
		...
		if (collection.collection in ReadCollectionTypes) {
			...
			schemaComposer.Subscription.addFields({ [eventName]: {
				type: subscriptionType,
				args: { event: subscriptionEventType },
	...
	for (const relation of schema.read.relations) {
		if (relation.related_collection) {
			...
			ReadableCollectionQuantifierFilterTypes[relation.collection]?.addFields({ [relation.field]: ReadableCollectionFilterTypes[relation.related_collection] });
			ReadableCollectionFilterTypes[relation.collection]?.addFields({ [relation.field]: ReadableCollectionFilterTypes[relation.related_collection] });
			...
			if (relation.meta?.one_field) {
				ReadableCollectionQuantifierFilterTypes[relation.related_collection]?.addFields({ [relation.meta.one_field]: ReadableCollectionQuantifierFilterTypes[relation.collection] });
				ReadableCollectionFilterTypes[relation.related_collection]?.addFields({ [relation.meta.one_field]: ReadableCollectionQuantifierFilterTypes[relation.collection] });
			...
			for (const collection of relation.meta.one_allowed_collections) {
				ReadableCollectionQuantifierFilterTypes[relation.collection]?.addFields({ [`${relation.field}__${collection}`]: ReadableCollectionFilterTypes[collection] });
				ReadableCollectionFilterTypes[relation.collection]?.addFields({ [`${relation.field}__${collection}`]: ReadableCollectionFilterTypes[collection] });
			}
		}
	}
	return {
		...
		ReadableCollectionFilterTypes
	};
}

//#endregion
export { getReadableTypes };