...
var UsersService = class UsersService extends ItemsService {
	...
	*/
	async checkPasswordPolicy(passwords) {
		...
		const { auth_password_policy: policyRegExString } = await settingsService.readSingleton({ fields: ["auth_password_policy"] });
	...
	*/
	...
	async registerUser(input) {
		if (input.verification_url && isUrlAllowed(input.verification_url, env["USER_REGISTER_URL_ALLOW_LIST"]) === false) {
			throw new InvalidPayloadError({ reason: `URL "${input.verification_url}" can't be used to verify registered users` });
		}
		...
		const settings = await settingsService.readSingleton({ fields: [
			"public_registration",
