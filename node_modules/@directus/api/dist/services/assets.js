	...
	zip(options) {
		...
		const complete = async () => {
			...
			for (const { id, folder, filename_download } of options.files) {
				...
				const version = file.modified_on ? (new Date(file.modified_on).getTime() / 1e3).toFixed() : undefined;
	...
	async getAsset(id, transformation, range, deferStream = false) {
		...
		*/
		...
		if (!systemPublicKeys.includes(id) && this.accountability && this.accountability.admin !== true) {
			...
			if (!accessAllowed) {
				throw new ForbiddenError({ reason: `You don't have permission to perform "read" for collection "directus_files" or it does not exist.` });
			}
		...
		const version = modifiedOn ? (modifiedOn.getTime() / 1e3).toFixed() : undefined;
		if (type && transforms.length > 0 && SUPPORTED_IMAGE_TRANSFORM_FORMATS.includes(type)) {
			...
			const exists$1 = await storage.location(file.storage).exists(assetFilename);
			if (maybeNewFormat) {
			...
			if (transforms.find((transform$1) => transform$1[0] === "rotate") === undefined) transformer.rotate();
			try {
