...
var TusDataStore = class extends DataStore {
	...
	async create(upload) {
		...
		if (upload.metadata["id"]) {
			existingFile = await knex.select("folder", "filename_download", "filename_disk", "title", "description", "metadata", "tus_id").from("directus_files").andWhere({ id: upload.metadata["id"] }).first() ?? null;
		...
		try {
			...
			logger.warn(err);
			if (isReplacement) {
				await filesItemsService.updateOne(primaryKey, {
					tus_id: null,
					tus_data: null
				}, { emitEvents: false });
			} else {
	...
	async deleteExpired() {
		...
		for (const fileData of uploadFiles) {
			if (fileData && fileData.tus_data && this.getExpiration() > 0 && fileData.tus_data["size"] !== fileData.tus_data["offset"] && fileData.modified_on) {
	...
	async getFileById(tus_id) {
		const sudoFilesItemsService = new ItemsService("directus_files", { schema: this.schema });
		const results = await sudoFilesItemsService.readByQuery({ filter: {
			tus_id: { _eq: tus_id },
