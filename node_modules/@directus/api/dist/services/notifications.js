...
var NotificationsService = class extends ItemsService {
	constructor(options) {
	...
	async sendEmail(data) {
		if (data.recipient) {
			...
			const user = await usersService.readOne(data.recipient, { fields: [
				"id",
				"email",
				"email_notifications",
				"role"
			...
			if (user["email"] && user["email_notifications"] === true) {
				...
				mailService.send({
					template: {
						name: "base",
						data: app_access ? {
							url: manageUserAccountUrl,
							html
						} : { html }
					},
					to: user["email"],
					subject: data.subject
				}, { defaultTemplateData: await mailService.getDefaultTemplateData() }).catch((error) => {
