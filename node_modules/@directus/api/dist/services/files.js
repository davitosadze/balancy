...
var FilesService = class FilesService extends ItemsService {
	...
	*/
	async checkUniqueFilename(filename) {
		const existingFile = await this.knex.select("filename_disk").from("directus_files").where({ filename_disk: filename }).first();
		if (existingFile) {
	...
	*/
	async uploadOne(stream, data, primaryKey, opts) {
		...
		if (primaryKey !== undefined) {
			existingFile = await this.knex.select("folder", "filename_download", "filename_disk", "title", "description", "metadata", "storage").from("directus_files").where({ id: primaryKey }).first() ?? null;
		}
		const payload = {
		...
		await sudoFilesItemsService.updateOne(primaryKey, {
			...payload,
			...metadata
		}, { emitEvents: false });
	...
	*/
	async importOne(importURL, body, options = {}) {
		...
		if (globalMimeTypeAllowed === false) {
			throw new InvalidPayloadError({ reason: `File content type "${mimeType}" is not allowed for upload by your global file type restrictions` });
		}
		...
		if (filterMimeType && filterMimeType.length > 0) {
			...
			if (interfaceMimeTypeAllowed === false) {
				throw new InvalidPayloadError({ reason: `File content type "${mimeType}" is not allowed for upload by this field's file type restrictions` });
			}
		}
		const payload = {
	...
	*/
	async updateMany(keys, data, opts = {}) {
		...
		if (keys.length > 1 && data.filename_disk) {
			opts.preMutationError = new InvalidPayloadError({ reason: "\"filename_disk\" cannot be modified in bulk operations" });
		}
	...
	*/
	async deleteMany(keys) {
		...
		const files = await sudoFilesItemsService.readMany(keys, {
			fields: [
				"id",
				"storage",
				"filename_disk"
	...
	async readByQuery(query, opts) {
		...
		if (RESUMABLE_UPLOADS.ENABLED === true) {
			...
			if (!filteredQuery.filter) {
				...
				filteredQuery.filter = { _and: [filteredQuery.filter, filterPartialUploads] };
			}
		}
...
function decompressResponse(stream, headers) {
	...
	function isDeflateAlgorithm(data) {
		const DEFLATE_ALGORITHM_HEADER = 8;
