...
//#region src/services/specifications.ts
const env = useEnv();
var SpecificationService = class {
	accountability;
	knex;
	schema;
	oas;
...
var OASSpecsService = class {
	...
	async generate(host) {
		...
		if (tags) spec$1.tags = tags;
		if (components) spec$1.components = components;
		return spec$1;
	}
		...
		for (const tag of tags) {
			...
			if (isSystem) {
				for (const [path, pathItem] of Object.entries(spec.paths)) {
					for (const [method, operation] of Object.entries(pathItem)) {
						if (operation.tags?.includes(tag.name)) {
							...
							const hasPermission = this.accountability?.admin === true || "x-collection" in tag === false || !!permissions.find((permission) => permission.collection === tag["x-collection"] && permission.action === this.getActionForMethod(method));
							if (hasPermission) {
				...
				for (const method of methods) {
					const hasPermission = this.accountability?.admin === true || !!permissions.find((permission) => permission.collection === collection && permission.action === this.getActionForMethod(method));
					if (hasPermission) {
						...
						if (listBase?.[method]) {
							paths[`/items/${collection}`][method] = mergeWith(cloneDeep(listBase[method]), {
								...
								requestBody: ["get", "delete"].includes(method) ? undefined : { content: { "application/json": { schema: { oneOf: [{
									type: "array",
									items: { $ref: `#/components/schemas/${tag.name}` }
								}, { $ref: `#/components/schemas/${tag.name}` }] } } } },
								responses: { "200": {
									description: "Successful request",
									content: method === "delete" ? undefined : { "application/json": { schema: { properties: { data: schema.collections[collection]?.singleton ? { $ref: `#/components/schemas/${tag.name}` } : {
										type: "array",
										items: { $ref: `#/components/schemas/${tag.name}` }
									} } } } }
								} }
						...
						if (detailBase?.[method]) {
							paths[`/items/${collection}/{id}`][method] = mergeWith(cloneDeep(detailBase[method]), {
								...
								requestBody: ["get", "delete"].includes(method) ? undefined : { content: { "application/json": { schema: { $ref: `#/components/schemas/${tag.name}` } } } },
								responses: { "200": { content: method === "delete" ? undefined : { "application/json": { schema: { properties: { data: { $ref: `#/components/schemas/${tag.name}` } } } } } } }
							}, (obj, src) => {
								if (Array.isArray(obj)) return obj.concat(src);
								return undefined;
							});
	...
	async generateComponents(schema, tags) {
		...
		for (const [name, schema$1] of Object.entries(spec.components?.schemas ?? {})) {
		...
		const collections = Object.values(schema.collections);
		for (const collection of collections) {
			const tag = tags.find((tag$1) => tag$1["x-collection"] === collection.collection);
			if (!tag) continue;
			const isSystem = isSystemCollection(collection.collection);
			...
			if (isSystem) {
				...
				for (const field of fieldsInCollection) {
					schemaComponent.properties[field.field] = cloneDeep(spec.components.schemas[tag.name].properties[field.field]) || this.generateField(schema, collection.collection, field, tags);
				}
	...
	generateField(schema, collection, field, tags) {
		...
		if (!relation) {
			...
			if (relationType === "m2o") {
				...
				if (!relatedTag || !relation.related_collection || relation.related_collection in schema.collections === false) {
				...
				propertyObject.oneOf = [{ ...this.fieldTypes[relatedPrimaryKeyField.type] }, { $ref: `#/components/schemas/${relatedTag.name}` }];
				...
				propertyObject.items = { oneOf: [{ ...this.fieldTypes[relatedPrimaryKeyField.type] }, { $ref: `#/components/schemas/${relatedTag.name}` }] };
			} else if (relationType === "a2o") {
				const relatedTags = tags.filter((tag) => relation.meta.one_allowed_collections.includes(tag["x-collection"]));
				propertyObject.type = "array";
				propertyObject.items = { oneOf: [{ type: "string" }, ...relatedTags.map((tag) => ({ $ref: `#/components/schemas/${tag.name}` }))] };
			}
		}
		return propertyObject;
	}
	fieldTypes = {
		...
		"geometry.Point": { type: "object" },
		"geometry.LineString": { type: "object" },
		"geometry.Polygon": { type: "object" },
		"geometry.MultiPoint": { type: "object" },
		"geometry.MultiLineString": { type: "object" },
		"geometry.MultiPolygon": { type: "object" }
	};
};
var GraphQLSpecsService = class {
	accountability;
	knex;
	schema;
	items;
	...
	constructor(options) {
		...
		this.knex = options.knex || database_default();
		this.schema = options.schema;
		this.items = new GraphQLService({
			...options,
			scope: "items"
		});
		this.system = new GraphQLService({
			...options,
			scope: "system"
		});
	}
