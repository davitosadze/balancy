...
var PayloadService = class {
	...
	transformers = {
		...
		async "date-created"({ action, value, helpers, overwriteDefaults }) {
			if (action === "create") return new Date(overwriteDefaults ? overwriteDefaults._date : helpers.date.writeTimestamp(new Date().toISOString()));
			return value;
		},
		async "date-updated"({ action, value, helpers, overwriteDefaults }) {
			if (action === "update") return new Date(overwriteDefaults ? overwriteDefaults._date : helpers.date.writeTimestamp(new Date().toISOString()));
			return value;
		},
	...
	async processA2O(data, opts) {
		...
		for (const relation of relationsToProcess) {
			...
			if (!relatedCollection) {
				throw new InvalidPayloadError({ reason: `Can't update nested record "${relation.collection}.${relation.field}" without field "${relation.collection}.${relation.meta.one_collection_field}" being set` });
			}
			const allowedCollections = relation.meta.one_allowed_collections;
			if (allowedCollections.includes(relatedCollection) === false) {
				throw new InvalidPayloadError({ reason: `"${relation.collection}.${relation.field}" can't be linked to collection "${relatedCollection}"` });
			}
			...
			const exists = hasPrimaryKey && !!await this.knex.select(relatedPrimaryKeyField).from(relatedCollection).where({ [relatedPrimaryKeyField]: relatedPrimaryKey }).first();
			if (exists) {
	...
	*/
	async processM2O(data, opts) {
		...
		for (const relation of relationsToProcess) {
			...
			const hasPrimaryKey = relatedPrimaryKeyField in relatedRecord;
			let relatedPrimaryKey = relatedRecord[relatedPrimaryKeyField];
			const exists = hasPrimaryKey && !!await this.knex.select(relatedPrimaryKeyField).from(relation.related_collection).where({ [relatedPrimaryKeyField]: relatedPrimaryKey }).first();
			if (exists) {
				const { [relatedPrimaryKeyField]: _,...record } = relatedRecord;
				if (Object.keys(record).length > 0) {
					await service.updateOne(relatedPrimaryKey, record, {
		...
		for (const relation of relationsToProcess) {
			...
			if (!field || Array.isArray(field)) {
				...
				for (let i = 0; i < updates.length; i++) {
					...
					let existingRecord;
					if (relatedId) {
						existingRecord = await this.knex.select(relatedPrimaryKeyField, relation.field).from(relation.collection).where({ [relatedPrimaryKeyField]: relatedId }).first();
					}
