...
var VersionsService = class VersionsService extends ItemsService {
	...
	async validateCreateData(data) {
		...
		if (!existingCollection.meta?.versioning) {
			throw new UnprocessableContentError({ reason: `Content Versioning is not enabled for collection "${data["collection"]}"` });
		}
		...
		if (existingVersions[0]["count"] > 0) {
			throw new UnprocessableContentError({ reason: `Version "${data["key"]}" already exists for item "${data["item"]}" in collection "${data["collection"]}"` });
		}
	}
	...
	async createMany(data, opts) {
		...
		for (const item of data) {
			...
			if (keyCombos.has(keyCombo)) {
				throw new UnprocessableContentError({ reason: `Cannot create multiple versions on "${item["item"]}" in collection "${item["collection"]}" with the same key "${item["key"]}"` });
			}
	...
	async updateMany(keys, data, opts) {
		...
		if ("key" in data) {
			...
			for (const pk of keys) {
				...
				if (keyCombos.has(keyCombo)) {
					throw new UnprocessableContentError({ reason: `Cannot update multiple versions on "${item}" in collection "${collection}" to the same key "${data["key"]}"` });
				}
				keyCombos.add(keyCombo);
				...
				if (existingVersions[0]["count"] > 0) {
					throw new UnprocessableContentError({ reason: `Version "${data["key"]}" already exists for item "${item}" in collection "${collection}"` });
				}
			}
		}
	...
	async promote(version, mainHash, fields) {
		...
		if (!delta) {
			throw new UnprocessableContentError({ reason: `No changes to promote` });
		}
		const { outdated } = await this.verifyHash(collection, item, mainHash);
		if (outdated) {
			throw new UnprocessableContentError({ reason: `Main item has changed since this version was last updated` });
		}
