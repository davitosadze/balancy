...
//#region src/services/relations.ts
const env = useEnv();
var RelationsService = class {
	...
	async readAll(collection, opts, bypassCache) {
		...
		const metaReadQuery = { limit: -1 };
		if (collection) {
			metaReadQuery.filter = { many_collection: { _eq: collection } };
		}
		const metaRows = [...await this.relationsItemService.readByQuery(metaReadQuery, opts), ...systemRelationRows].filter((metaRow) => {
	...
	async readOne(collection, field) {
		...
		const metaRow = await this.relationsItemService.readByQuery({
			limit: 1,
			filter: { _and: [{ many_collection: { _eq: collection } }, { many_field: { _eq: field } }] }
		});
	...
	*/
	async createOne(relation, opts) {
		...
		if (!fieldSchema) {
			throw new InvalidPayloadError({ reason: `Field "${relation.field}" doesn't exist in collection "${relation.collection}"` });
		}
		if (collectionSchema.primary === relation.field) {
			throw new InvalidPayloadError({ reason: `Field "${relation.field}" in collection "${relation.collection}" is a primary key` });
		}
		...
		const existingRelation = this.schema.relations.find((existingRelation$1) => existingRelation$1.collection === relation.collection && existingRelation$1.field === relation.field);
		if (existingRelation) {
			throw new InvalidPayloadError({ reason: `Field "${relation.field}" in collection "${relation.collection}" already has an associated relationship` });
		}
			...
			await transaction(this.knex, async (trx) => {
				if (relation.related_collection) {
					await trx.schema.alterTable(relation.collection, async (table) => {
						...
						const builder = table.foreign(relation.field, constraintName).references(`${relation.related_collection}.${this.schema.collections[relation.related_collection].primary}`);
				...
				await relationsItemService.createOne(metaRow, { bypassEmitAction: (params) => opts?.bypassEmitAction ? opts.bypassEmitAction(params) : nestedActionEvents.push(params) });
			});
		} finally {
	...
	*/
	async updateOne(collection, field, relation, opts) {
		...
		const existingRelation = this.schema.relations.find((existingRelation$1) => existingRelation$1.collection === collection && existingRelation$1.field === field);
		if (!existingRelation) {
			throw new InvalidPayloadError({ reason: `Field "${field}" in collection "${collection}" doesn't have a relationship.` });
		}
		const runPostColumnChange = await this.helpers.schema.preColumnChange();
		this.helpers.schema.preRelationChange(relation);
		const nestedActionEvents = [];
			...
			await transaction(this.knex, async (trx) => {
				if (existingRelation.related_collection) {
					await trx.schema.alterTable(collection, async (table) => {
						...
						this.alterType(table, relation, fieldSchema.nullable);
						const builder = table.foreign(field, constraintName || undefined).references(`${existingRelation.related_collection}.${this.schema.collections[existingRelation.related_collection].primary}`);
						if (relation.schema?.on_delete) {
							builder.onDelete(relation.schema.on_delete);
						}
						if (relation.schema?.on_update) {
				...
				if (relation.meta) {
					if (existingRelation?.meta) {
						await relationsItemService.updateOne(existingRelation.meta.id, relation.meta, { bypassEmitAction: (params) => opts?.bypassEmitAction ? opts.bypassEmitAction(params) : nestedActionEvents.push(params) });
					} else {
	...
	async deleteOne(collection, field, opts) {
		...
		try {
			await transaction(this.knex, async (trx) => {
				...
				if (existingRelation.schema?.constraint_name && constraintNames.includes(existingRelation.schema.constraint_name)) {
				...
				if (existingRelation.meta) {
					await trx("directus_relations").delete().where({
	...
	*/
	stitchRelations(metaRows, schemaRows) {
		...
		*/
		const remainingMetaRows = metaRows.filter((meta) => {
	...
	*/
	async filterForbidden(relations) {
		...
		return relations.filter((relation) => {
			...
			if (relation.meta?.one_allowed_collections && relation.meta?.one_allowed_collections.every((collection) => allowedCollections.includes(collection)) === false) {
				collectionsAllowed = false;
			}
			if (!allowedFields[relation.collection] || allowedFields[relation.collection]?.includes("*") === false && allowedFields[relation.collection]?.includes(relation.field) === false) {
				fieldsAllowed = false;
			}
			if (relation.related_collection && relation.meta?.one_field && (!allowedFields[relation.related_collection] || allowedFields[relation.related_collection]?.includes("*") === false && allowedFields[relation.related_collection]?.includes(relation.meta.one_field) === false)) {
				fieldsAllowed = false;
			}
