...
var AuthenticationService = class {
	...
	*/
	async login(providerName = DEFAULT_AUTH_PROVIDER, payload, options) {
		...
		const { auth_login_attempts: allowedAttempts } = await settingsService.readSingleton({ fields: ["auth_login_attempts"] });
		...
		if (!user.tfa_secret) {
			const roleEnforcement = await this.knex.select("directus_policies.enforce_tfa").from("directus_users").leftJoin("directus_roles", "directus_users.role", "directus_roles.id").leftJoin("directus_access", "directus_roles.id", "directus_access.role").leftJoin("directus_policies", "directus_access.policy", "directus_policies.id").where("directus_users.id", user.id).where("directus_policies.enforce_tfa", true).first();
			if (roleEnforcement) {
	...
	async refresh(refreshToken, options) {
		...
		}).from("directus_sessions AS s").leftJoin("directus_users AS u", "s.user", "u.id").leftJoin("directus_shares AS d", "s.share", "d.id").where("s.token", refreshToken).andWhere("s.expires", ">=", new Date()).andWhere((subQuery) => {
		...
		if (options?.session) {
			...
			await this.knex("directus_sessions").update({
	...
	async updateStatefulSession(sessionRecord, oldSessionToken, newSessionToken, sessionExpiration) {
		...
		const updatedSession = await this.knex("directus_sessions").update({
