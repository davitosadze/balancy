...
var FlowManager = class {
		...
		for (const flow of flowTrees) {
			if (flow.trigger === "event") {
				...
				if (flow.options?.["scope"]) {
					events = toArray(flow.options["scope"]).map((scope) => {
						if ([
							"items.create",
							"items.update",
							"items.delete"
				...
				const handler = async (data, context) => {
					...
					if (accountability.admin === false) {
						...
						if (Array.isArray(targetKeys) && targetKeys.length > 0) {
							...
							const keys = await service.readMany(targetKeys, { fields: [primaryField] }, { emitEvents: false });
	...
	async executeFlow(flow, data = null, context = {}) {
		...
		if ((flow.trigger === "manual" || flow.trigger === "webhook") && flow.options["async"] !== true && flow.options["error_on_reject"] === true && lastOperationStatus === "reject") {
		...
		if (flow.options["return"] === "$all") {
			...
			return get$1(keyedData, flow.options["return"]);
		}
		return undefined;
	}
	async executeOperation(operation, keyedData, context = {}) {
		...
		if (operation.type === "log") {
			optionData = redactObject(keyedData, { keys: [
				[
					"**",
					"headers",
					"authorization"
