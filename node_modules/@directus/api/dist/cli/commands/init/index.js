...
async function init() {
	...
	async function trySeed() {
		const credentials$1 = await inquirer.prompt(databaseQuestions[dbClient].map((question) => question({
			client: dbClient,
			filepath: rootPath
		})));
		const db$1 = createDBConnection(dbClient, credentials$1);
		try {
			await runSeed(db$1);
			await run(db$1, "latest", false);
		} catch (err) {
