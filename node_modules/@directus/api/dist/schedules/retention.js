...
const retentionLockTimeout = 10 * 60 * 1e3;
...
async function schedule() {
	const env$1 = useEnv();
	if (!toBoolean(env$1["RETENTION_ENABLED"])) {
		return false;
	}
	if (!validateCron(String(env$1["RETENTION_SCHEDULE"]))) {
		return false;
	}
	if (!ACTIVITY_RETENTION_TIMEFRAME || ACTIVITY_RETENTION_TIMEFRAME && REVISIONS_RETENTION_TIMEFRAME && ACTIVITY_RETENTION_TIMEFRAME > REVISIONS_RETENTION_TIMEFRAME) {
		retentionTasks.push({
