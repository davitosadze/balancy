...
function buildTranslationsAliasField(languagesFields) {
	return {
		field: "translations",
		type: "alias",
		meta: {
...
async function generateTranslations(input, options) {
	...
	const result = await transaction(options.knex, async (trx) => {
		let currentSchema = options.schema ?? await getSchema({
			database: trx,
			bypassCache: true
		});
		const getServiceOptions = (schema) => ({
		...
		const validateExistingTranslationsCollection = async () => {
			const invalidCollectionError = new InvalidPayloadError({ reason: `Collection "${translationsCollection}" is not a valid translations collection for "${collection}"` });
			let parentRelations = currentSchema.relations.filter((relation) => relation.collection === translationsCollection && relation.related_collection === collection && (relation.meta?.one_collection === null || relation.meta?.one_collection === collection) && (relation.meta?.many_collection === null || relation.meta?.many_collection === translationsCollection) && typeof relation.meta?.one_field === "string" && relation.meta.one_field.length > 0 && typeof relation.meta?.junction_field === "string" && relation.meta.junction_field.length > 0);
			if (parentRelations.length === 0) {
				const orphanedRelations = currentSchema.relations.filter((relation) => relation.collection === translationsCollection && relation.related_collection === collection && (relation.meta?.one_collection === null || relation.meta?.one_collection === collection) && (relation.meta?.many_collection === null || relation.meta?.many_collection === translationsCollection) && relation.meta?.one_field === null && typeof relation.meta?.junction_field === "string" && relation.meta.junction_field.length > 0);
				if (orphanedRelations.length === 1) {
					const orphanedRelation = orphanedRelations[0];
					const languageFkField$1 = orphanedRelation.meta.junction_field;
					const orphanedLanguageRelation = currentSchema.relations.find((relation) => relation.collection === translationsCollection && relation.field === languageFkField$1 && relation.meta?.junction_field === orphanedRelation.field && typeof relation.related_collection === "string" && relation.related_collection.length > 0);
					...
					parentRelations = currentSchema.relations.filter((relation) => relation.collection === translationsCollection && relation.related_collection === collection && relation.meta?.one_field === "translations" && typeof relation.meta?.junction_field === "string" && relation.meta.junction_field.length > 0);
				}
			}
		...
		if (existingTranslationsCollection) {
			...
			if (newFieldNames.length > 0) {
				const sourceFieldsService = new FieldsService(getServiceOptions(currentSchema));
				const sourceFields$1 = await Promise.all(newFieldNames.map(async (fieldName) => await sourceFieldsService.readOne(collection, fieldName)));
				validateFieldsEligibility(sourceFields$1);
				const clonedFields$1 = cloneFields({
					fields: newFieldNames,
					sourceFields: sourceFields$1
				});
				const allParentFields = await trx.from("directus_fields").where({ collection }).select("field", "sort");
				const parentSortMap = new Map(allParentFields.map((f) => [f.field, f.sort ?? Infinity]));
				const existingTransFields = await trx.from("directus_fields").where({ collection: translationsCollection }).select("field", "sort");
				const systemFieldNames = new Set([
					"id",
					parentRelation.field,
					languageRelation.field
				]);
				const maxSystemSort = existingTransFields.filter((f) => systemFieldNames.has(f.field)).reduce((max, f) => Math.max(max, f.sort ?? 0), 0);
		...
		const onDeleteSchema = { on_delete: "SET NULL" };
