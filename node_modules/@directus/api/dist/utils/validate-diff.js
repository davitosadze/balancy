...
//#region src/utils/validate-diff.ts
const deepDiffSchema = Joi.object({
	kind: Joi.string().valid(...Object.values(DiffKind)).required(),
	path: Joi.array().items(Joi.alternatives().try(Joi.string(), Joi.number())),
	lhs: Joi.any().when("kind", {
		is: [DiffKind.NEW, DiffKind.ARRAY],
		then: Joi.optional(),
	...
	index: Joi.number().when("kind", {
		is: DiffKind.ARRAY,
		then: Joi.required()
	}),
	item: Joi.link("#deepdiff").when("kind", {
		is: DiffKind.ARRAY,
		then: Joi.required()
	})
}).id("deepdiff");
