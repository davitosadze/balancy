...
function getSnapshotDiff(current, after) {
	const diffedSnapshot = {
		collections: [...current.collections.map((currentCollection) => {
			...
			const currentCollection = current.collections.find((currentCollection$1) => currentCollection$1.collection === afterCollection.collection);
		...
		fields: [...current.fields.map((currentField) => {
			...
			if (afterField && currentField.type !== afterField.type && (currentField.type === "alias" || afterField.type === "alias")) {
				return {
			...
			if (currentField && currentField.type !== afterField.type && (currentField.type === "alias" || afterField.type === "alias")) {
		...
		relations: [...current.relations.map((currentRelation) => {
			...
			const currentRelation = current.relations.find((currentRelation$1) => currentRelation$1.collection === afterRelation.collection && afterRelation.field === currentRelation$1.field);
