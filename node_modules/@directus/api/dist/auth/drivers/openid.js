...
var OpenIDAuthDriver = class extends LocalAuthDriver {
	...
	async fetchUserId(identifier) {
		const user = await this.knex.select("id").from("directus_users").whereRaw("LOWER(??) = ?", ["external_identifier", identifier.toLowerCase()]).first();
		return user?.id;
	}
	async getUserID(payload) {
		...
		if (userId) {
			...
			if (Object.values(updatedUserPayload$1).some((value) => value !== undefined)) {
		...
		if (authData?.["refreshToken"]) {
			try {
				...
				if (tokenSet.refresh_token) {
					...
					await usersService.updateOne(user.id, { auth_data: JSON.stringify({ refreshToken: tokenSet.refresh_token }) });
				}
			} catch (e) {
				throw handleError(e);
			}
...
function createOpenIDAuthRouter(providerName) {
	...
	router.get("/callback", async_handler_default(async (req, res, next) => {
		...
		if (redirect) {
			if (authMode === "session") {
				res.cookie(env$1["SESSION_COOKIE_NAME"], accessToken, SESSION_COOKIE_OPTIONS);
			} else {
				res.cookie(env$1["REFRESH_TOKEN_COOKIE_NAME"], refreshToken, REFRESH_COOKIE_OPTIONS);
			}
