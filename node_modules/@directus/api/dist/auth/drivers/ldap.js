...
var LDAPAuthDriver = class extends AuthDriver {
	...
	constructor(options, config) {
		...
		if (bindDn === undefined || bindPassword === undefined || !userDn || !provider || !clientUrl && !config["client"]?.socketPath) {
	...
	async fetchUserId(userDn) {
		const user = await this.knex.select("id").from("directus_users").orWhereRaw("LOWER(??) = ?", ["external_identifier", userDn.toLowerCase()]).first();
		return user?.id;
	}
	async getUserID(payload) {
		...
		if (groupDn) {
			...
			if (userGroups.length) {
				userRole = await this.knex.select("id").from("directus_roles").whereRaw(`LOWER(??) IN (${userGroups.map(() => "?")})`, ["name", ...userGroups.map((group) => group.toLowerCase())]).first();
			}
		}
		...
		if (userId) {
			let emitPayload = {};
			if (groupDn) {
				emitPayload = { role: userRole?.id ?? defaultRoleId ?? null };
			}
			if (syncUserInfo) {
				emitPayload = {
					...emitPayload,
			...
			const usersService = this.getUsersService(schema$1);
			await usersService.updateOne(userId, updatedUserPayload$1);
			return userId;
		}
		if (!userInfo) {
			throw new InvalidCredentialsError();
...
const handleError = (e) => {
	if (e instanceof InappropriateAuthError || e instanceof InvalidCredentialsError$1 || e instanceof InsufficientAccessError) {
