...
const getExtensionsSettings = async ({ local, module, registry }) => {
	...
	for (const [folder, extension] of local.entries()) {
		...
		if (existingSettings$1) {
			if (extension.type === "bundle") {
				updateBundleEntriesSettings(extension, existingSettings$1, localSettings);
			}
			continue;
		}
	...
	for (const [folder, extension] of module.entries()) {
		...
		if (!existingSettings$1) {
			...
			updateBundleEntriesSettings(extension, existingSettings$1, moduleSettings);
		}
	}
	for (const [folder, extension] of registry.entries()) {
		...
		if (!existingSettings$1) {
			...
			updateBundleEntriesSettings(extension, existingSettings$1, registrySettings);
		}
	}
