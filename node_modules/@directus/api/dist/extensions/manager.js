...
var ExtensionManager = class {
	...
	*/
	async install(versionId) {
		...
		const syncFolder = join$1(".registry", resolvedFolder);
	...
	*/
	async getAppExtensionChunk(name) {
		...
		if (!file) return null;
		const tempDir = join$1(env["TEMP_PATH"], "app-extensions");
	...
	*/
	async generateExtensionBundle() {
		...
		try {
			...
			const rollDirection = env$1["EXTENSIONS_ROLLDOWN"] ?? false ? rolldown : rollup;
	...
	async registerEndpointExtension(endpoint) {
		try {
			if (endpoint.sandbox?.enabled) {
				...
				const endpointInstance = await importFileUrl(endpointPath, import.meta.url, { fresh: true });
	...
	async registerOperationExtension(operation) {
		try {
			if (operation.sandbox?.enabled) {
				...
				const operationInstance = await importFileUrl(operationPath, import.meta.url, { fresh: true });
