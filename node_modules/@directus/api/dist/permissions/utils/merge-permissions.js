...
function mergePermissions(strategy, ...permissions) {
	...
	if (strategy === "intersection") {
		...
		const deduplicateSubpermissions = permissions.map((permissions$1) => {
...
function mergePermission(strategy, currentPerm, newPerm) {
	...
	if (newPerm.permissions) {
		if (currentPerm.permissions && Object.keys(currentPerm.permissions)[0] === logicalKey) {
			permissions = { [logicalKey]: [...currentPerm.permissions[logicalKey], newPerm.permissions] };
		} else if (currentPerm.permissions) {
			if (strategy === "or" && (isEqual(currentPerm.permissions, {}) || isEqual(newPerm.permissions, {}))) {
				permissions = {};
			} else {
				permissions = { [logicalKey]: [currentPerm.permissions, newPerm.permissions] };
			}
		} else {
			permissions = { [logicalKey]: [newPerm.permissions] };
		}
	}
	if (newPerm.validation) {
		if (currentPerm.validation && Object.keys(currentPerm.validation)[0] === logicalKey) {
			validation = { [logicalKey]: [...currentPerm.validation[logicalKey], newPerm.validation] };
		} else if (currentPerm.validation) {
			if (strategy === "or" && (isEqual(currentPerm.validation, {}) || isEqual(newPerm.validation, {}))) {
				validation = {};
			} else {
				validation = { [logicalKey]: [currentPerm.validation, newPerm.validation] };
			}
		} else {
			validation = { [logicalKey]: [newPerm.validation] };
		}
	}
