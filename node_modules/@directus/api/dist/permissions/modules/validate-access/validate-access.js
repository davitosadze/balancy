...
async function validateAccess(options, context) {
	...
	if (!access) {
		if (options.fields?.length ?? 0 > 0) {
			throw new ForbiddenError({ reason: `You don't have permissions to perform "${options.action}" for the field(s) ${options.fields.map((field) => `"${field}"`).join(", ")} in collection "${options.collection}" or it does not exist.` });
		}
