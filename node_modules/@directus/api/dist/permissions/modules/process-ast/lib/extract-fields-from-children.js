...
function extractFieldsFromChildren(collection, children, fieldMap, schema, path = []) {
	...
	for (const child of children) {
		...
		if (child.type === "a2o") {
			for (const [collection$1, children$1] of Object.entries(child.children)) {
				extractFieldsFromChildren(collection$1, children$1, fieldMap, schema, [...path, formatA2oKey(child.fieldKey, collection$1)]);
			}
			if (child.query) {
				for (const [collection$1, query] of Object.entries(child.query)) {
					extractFieldsFromQuery(collection$1, query, fieldMap, schema, [...path, formatA2oKey(child.fieldKey, collection$1)]);
				}
			}
