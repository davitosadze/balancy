...
function getCases(collection, permissions, requestedKeys) {
	...
	for (const { rule, fields } of rules) {
		if (requestedKeys.length > 0 && fields.has("*") === false && Array.from(fields).every((field) => requestedKeys.includes(field) === false)) {
			continue;
		}
	...
	const allowedFields = new Set(permissionsForCollection.filter((permission) => hasItemPermissions(permission) === false).map((permission) => permission.fields ?? []).flat());
	return {
		cases,
		caseMap,
		allowedFields
