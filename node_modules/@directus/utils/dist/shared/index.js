...
const messageConstructor$19 = ({ collection, field }) => `Field "${field}" in collection "${collection}" contains null values.`;
const ContainsNullValuesError = createError(ErrorCode.ContainsNullValues, messageConstructor$19, 400);
const ContentTooLargeError = createError(ErrorCode.ContentTooLarge, "Uploaded content is too large.", 413);
const messageConstructor$18 = (extensions) => {
	...
	if (typeof extensions.points === "number" && typeof extensions.duration === "number") {
		...
		const plural$1 = extensions.points !== 1 ? "s" : "";
		message.push(`Limit of ${extensions.points} email${plural$1} every ${duration}.`);
	}
	if (extensions.message) message.push(extensions.message);
	return message.join(" ");
};
const EmailLimitExceededError = createError(ErrorCode.EmailLimitExceeded, messageConstructor$18, 429);
const messageConstructor$17 = (ext) => {
	if (ext?.reason) return ext.reason;
	return `You don't have permission to access this.`;
};
const ForbiddenError = createError(ErrorCode.Forbidden, messageConstructor$17, 403);
const messageConstructor$16 = (extensions) => {
	return `Too many requests, retry after ${(0, import_ms.default)(extensions.reset.getTime() - Date.now())}.`;
};
const HitRateLimitError = createError(ErrorCode.RequestsExceeded, messageConstructor$16, 429);
...
const messageConstructor$15 = ({ collection, field, value }) => {
...
const InvalidForeignKeyError = createError(ErrorCode.InvalidForeignKey, messageConstructor$15, 400);
...
const messageConstructor$14 = ({ reason }) => `Invalid payload. ${reason}.`;
const InvalidPayloadError = createError(ErrorCode.InvalidPayload, messageConstructor$14, 400);
const messageConstructor$13 = ({ reason }) => `Invalid path parameter. ${reason}.`;
const InvalidPathParameterError = createError(ErrorCode.InvalidPathParameter, messageConstructor$13, 400);
...
const messageConstructor$12 = ({ reason }) => `Invalid query. ${reason}.`;
const InvalidQueryError = createError(ErrorCode.InvalidQuery, messageConstructor$12, 400);
const InvalidTokenError = createError(ErrorCode.InvalidToken, "Invalid token.", 403);
const messageConstructor$11 = ({ category }) => {
	return `${category} limit exceeded.`;
};
const LimitExceededError = createError(ErrorCode.LimitExceeded, messageConstructor$11, 403);
const messageConstructor$10 = (extensions) => `Invalid method "${extensions.current}" used. Should be one of ${extensions.allowed.map((method) => `"${method}"`).join(", ")}.`;
const MethodNotAllowedError = createError(ErrorCode.MethodNotAllowed, messageConstructor$10, 405);
const messageConstructor$9 = ({ collection, field }) => {
	let message = "Value ";
	if (field) message += `for field "${field}" `;
	if (collection) message += `in collection "${collection}" `;
...
const NotNullViolationError = createError(ErrorCode.NotNullViolation, messageConstructor$9, 400);
...
const messageConstructor$8 = ({ range }) => {
	return `Range ${`"${range.start ?? ""}-${range.end ?? ""}"`} is invalid or the file's size doesn't match the requested range.`;
};
const RangeNotSatisfiableError = createError(ErrorCode.RangeNotSatisfiable, messageConstructor$8, 416);
const messageConstructor$7 = ({ collection, field, value }) => {
	let message = "Value ";
	if (value) message += `"${value}" `;
	if (field) message += `for field "${field}" `;
	if (collection) message += `in collection "${collection}" `;
...
const RecordNotUniqueError = createError(ErrorCode.RecordNotUnique, messageConstructor$7, 400);
const messageConstructor$6 = ({ path }) => `Route ${path} doesn't exist.`;
const RouteNotFoundError = createError(ErrorCode.RouteNotFound, messageConstructor$6, 404);
const messageConstructor$5 = ({ service, reason }) => `Service "${service}" is unavailable. ${reason}.`;
const ServiceUnavailableError = createError(ErrorCode.ServiceUnavailable, messageConstructor$5, 503);
...
const messageConstructor$4 = (extensions) => `Can't process content. ${extensions.reason}.`;
const UnprocessableContentError = createError(ErrorCode.UnprocessableContent, messageConstructor$4, 422);
const messageConstructor$3 = (extensions) => `Unsupported media type "${extensions.mediaType}" in ${extensions.where}.`;
const UnsupportedMediaTypeError = createError(ErrorCode.UnsupportedMediaType, messageConstructor$3, 415);
const UserSuspendedError = createError(ErrorCode.UserSuspended, "User suspended.", 401);
const messageConstructor$2 = ({ collection, field, value }) => {
...
const ValueOutOfRangeError = createError(ErrorCode.ValueOutOfRange, messageConstructor$2, 400);
...
const ValueTooLongError = createError(ErrorCode.ValueTooLong, messageConstructor$1, 400);
const messageConstructor = ({ category, duration }) => `${category} timed out after ${(0, import_ms.default)(duration, { long: true })}.`;
const TimeoutError = createError(ErrorCode.OutOfTime, messageConstructor, 408);
